...
export const TextButton: React.FC<TextButtonProps> = ({
  ...
  const baseClasses =
    "inline-flex justify-center items-center shrink-0 cursor-pointer bg-transparent border-none";
  ...
  return (
    <button className={classes} onClick={onClick} disabled={disabled}>
      {leftIcon && <div className={iconSizeClasses[size]}>{leftIcon}</div>}
      <span
      ...
      {rightIcon && <div className={iconSizeClasses[size]}>{rightIcon}</div>}
    </button>
  );
};
